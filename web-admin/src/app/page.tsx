...
export default function DashboardPage() {
  ...
  const groupedMasterProducts = useMemo(() => {
    ...
    for (const prod of masterProductsList) {
      const baseName = getProductBaseName(prod.name) || prod.name || 'Product';
      const brand = (prod.brand || '').trim();
      const category = (prod.primary_category || '').trim();
      const key = `${brand.toLowerCase()}:::${category.toLowerCase()}:::${baseName.toLowerCase()}`;
      ...
      if (!map.has(key)) {
        map.set(key, {
          ...
          secondary_category: prod.secondary_category || '',
        ...
        if (!group.image_url && prod.image_url) {
          group.image_url = prod.image_url;
        }
      }
    }
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (activeTab === 'home-screen') {
        ...
        if (categoryProductsData.category_products) {
  ...
  const handleDeleteProduct = async (productId: string, productName?: string) => {
    ...
    if (!confirm(`Are you sure you want to delete ${label} from the master catalog?`)) return;
    try {
      await apiFetch(`/products/master/${productId}`, {
        method: 'DELETE',
      });
  ...
  // Delete Entire Product Group (all pack size variants)
  const handleDeleteProductGroup = async (group: any) => {
    if (!group.variants || group.variants.length <= 1) {
      return handleDeleteProduct(group.variants[0]?.id || group.primaryProduct?.id, group.baseName);
    }
    if (!confirm(`Are you sure you want to delete all ${group.variants.length} pack size variants of "${group.baseName}" from the master catalog?`)) return;
    try {
      for (const v of group.variants) {
        await apiFetch(`/products/master/${v.id}`, { method: 'DELETE' });
      }
      fetchData();
    } catch (err: any) {
      alert(err.message || 'Failed to delete product variants');
    }
  };

  // Delete Individual Variant from Modal
  ...
  const handleSaveMasterProduct = async (e?: React.FormEvent) => {
    ...
    if (!editMasterProduct || !token) return;
    ...
    try {
      ...
      if (editProdImageFile) {
        ...
        finalImageUrl = await uploadAdminImage(editProdImageFile);
  ...
  const handleSaveProductDetail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(err.message || 'Error saving product detail screen copy');
    } finally {
  ...
  const handleSaveOffersCouponsScreen = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(err.message || 'Error saving offers screen copy');
    } finally {
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-800/30 text-sm">
                    {locations.map((loc) => {
                      ...
                      return (
                        <tr key={loc.id} className={`hover:bg-slate-800/20 transition-colors ${isUnassigned ? 'bg-amber-500/5' : ''}`}>
                          ...
                          <td className="py-4 pr-3">
                            <select
                              className={`w-full max-w-xs h-9 px-2 bg-slate-950 border ${isUnassigned ? 'border-amber-500/70 focus:border-amber-400' : 'border-slate-800 focus:border-emerald-500'} text-slate-200 rounded-lg text-sm outline-none`}
                              value={loc.shop_id || ''}
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      const matchedArea = areas.find(a => a.name === selectedAreaName && (cityObj ? a.city_id === cityObj.id : true));
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 lg:gap-6 w-full">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-800/80">
                <div>
                  <h2 className="text-base sm:text-lg font-bold text-white flex items-center gap-2">
                    <MessageSquare className="w-5 h-5 text-emerald-400" /> Franchise & Partnership Leads ({franchiseRequests.length})
                  </h2>
                  <p className="text-xs text-slate-400 mt-0.5">Track prospective dark store partners, regional distributors & hub inquiries.</p>
                </div>
                <button onClick={fetchData} className="text-xs font-bold text-slate-400 hover:text-white px-3 py-1.5 bg-slate-950 rounded-xl border border-slate-800 w-fit cursor-pointer">
                  Refresh Leads
                </button>
              </div>
              ...
              <div className="space-y-3.5">
                {franchiseRequests
                  ...
                  .map((req) => {
                    const st = req.status || 'new';
                    return (
                      <div key={req.id} className="p-4 sm:p-5 border border-slate-800/80 rounded-2xl bg-slate-950/40 hover:border-slate-700/80 transition-all space-y-3">
                        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2">
                          <div>
                            <div className="flex items-center gap-2.5 flex-wrap">
                              <h4 className="font-bold text-sm sm:text-base text-white">{req.name}</h4>
                              <span className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full border ${
                                st === 'converted' ? 'bg-emerald-500/15 border-emerald-500/30 text-emerald-400' :
                                st === 'contacted' ? 'bg-amber-500/15 border-amber-500/30 text-amber-400' :
                                st === 'rejected' ? 'bg-rose-500/15 border-rose-500/30 text-rose-400' :
                                'bg-blue-500/15 border-blue-500/30 text-blue-400'
                              }`}>
                                {st === 'converted' ? '✓ Partner Converted' : st === 'contacted' ? '⏳ Under Discussion' : st === 'rejected' ? '✕ Closed / Not Eligible' : '✨ New Lead'}
                              </span>
                            ...
                            <div className="flex flex-wrap items-center gap-3 text-xs text-slate-400 mt-1.5 font-medium">
                              <a href={`tel:${req.phone}`} className="hover:text-emerald-400 transition-colors flex items-center gap-1">
                                📞 +91 {req.phone}
                              </a>
                              {req.email ? (
                                <a href={`mailto:${req.email}`} className="hover:text-cyan-400 transition-colors flex items-center gap-1">
                                  ✉️ {req.email}
                                </a>
                              ) : null}
                        ...
                        {req.message && (
                          <div className="p-3 bg-slate-950/80 rounded-xl border border-slate-850 text-xs text-slate-300 leading-relaxed">
                            <span className="text-[10px] font-bold text-slate-500 uppercase block mb-1">Notes / Discussion</span>
                            {req.message}
                          </div>
                        )}
                ...
                {franchiseRequests.length === 0 && (
                  <div className="py-12 text-center text-slate-500 space-y-2">
                    ...
                    <p className="text-xs text-slate-600">Use the form on the right to manually generate and record franchise leads.</p>
                  </div>
                )}
              </div>
            </section>
            ...
            <section className="lg:col-span-5 xl:col-span-4 bg-slate-900/40 rounded-2xl sm:rounded-3xl p-4 sm:p-5 md:p-6 border border-slate-800/80 backdrop-blur-xl shadow-xl h-max space-y-4">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">Manually record prospective franchise partners and hub discussions.</p>
              </div>

              <form onSubmit={handleCreateFranchiseLead} className="space-y-3.5">
                <div>
                  <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wide block mb-1">Partner Full Name *</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wide block mb-1">Mobile Number *</label>
                  <input
                    type="tel"
                    required
                    maxLength={10}
                ...
                <div>
                  <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wide block mb-1">Email ID (Optional)</label>
                  <input
                    type="email"
                ...
                <div>
                  <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wide block mb-1">Target City / Territory *</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wide block mb-1">Investment Capacity (Optional)</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wide block mb-1">Initial Lead Stage</label>
                  <select
                ...
                <div>
                  <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wide block mb-1">Meeting Notes & Discussion</label>
                  <textarea
                    rows={3}
                  ...
                  <div className="divide-y divide-slate-800/40 max-h-56 overflow-y-auto custom-scrollbar pr-1">
                    {adminStates
                      ...
                      .map((state) => {
                        ...
                        return (
                          <div key={state.id} className="flex justify-between items-center py-2.5 px-2 hover:bg-slate-800/20 rounded-lg transition-colors">
                            <div className="flex items-center gap-2">
                  ...
                  <div className="divide-y divide-slate-800/40 max-h-60 overflow-y-auto custom-scrollbar pr-1">
                    {adminDistricts
                      ...
                      .map((district) => {
                        ...
                        return (
                          <div key={district.id} className="flex justify-between items-center py-2.5 px-2 hover:bg-slate-800/20 rounded-lg transition-colors">
                            <div>
                  ...
                  <div className="divide-y divide-slate-800/40 max-h-56 overflow-y-auto custom-scrollbar pr-1">
                    {cities
                      ...
                      .map((city) => {
                        ...
                        return (
                          <div key={city.id} className="flex justify-between items-center py-2.5 px-2 hover:bg-slate-800/20 rounded-lg transition-colors">
                            <div className="flex items-center gap-2">
                  ...
                  <div className="divide-y divide-slate-800/40 max-h-60 overflow-y-auto custom-scrollbar pr-1">
                    {areas
                      ...
                      .map((area) => {
                        ...
                        return (
                          <div key={area.id} className="flex justify-between items-center py-2.5 px-2 hover:bg-slate-800/20 rounded-lg transition-colors">
                            <div>
                              <div className="flex items-center gap-2">
              ...
              <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-2.5 py-0.5 rounded-full font-bold">
                    {catalogViewMode === 'grouped' ? `${groupedMasterProducts.length} Products` : `${masterProductsList.length} SKUs`}
                  </span>
                </div>
                <div className="flex items-center gap-3">
                  {/* View Mode Toggle */}
                  <div className="bg-slate-950 p-1 rounded-xl border border-slate-800 flex items-center gap-1">
                    ...
                    >
                      Grouped ({groupedMasterProducts.length})
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      All SKUs ({masterProductsList.length})
                    </button>
                  </div>
                  <button onClick={fetchData} className="text-xs font-bold text-slate-400 hover:text-white transition-colors cursor-pointer px-2 py-1">Refresh</button>
                </div>
              </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="text-xs font-bold text-slate-400 uppercase tracking-wider">
                      ...
                      <th className="pb-3 sticky top-0 bg-[#131c2e] border-b border-slate-800/85 z-10">Pack Variants</th>
                            ...
                            <td className="py-4 pr-3">
                              <select
                                ...
                                onChange={async (e) => {
                                  ...
                                  for (const v of group.variants) {
                                    await handleUpdateProductCategory(v.id, newCat);
                                  }
                                }}
                            ...
                            <td className="py-4 pr-3">
                              <select
                                ...
                                onChange={async (e) => {
                                  ...
                                  for (const v of group.variants) {
                                    await handleUpdateProductSubcategory(v.id, newSub);
                                  }
                                }}
                            ...
                            <td className="py-4 text-right">
                              <div className="flex items-center justify-end gap-1">
                                <button
                                  ...
                                  title="Edit All Details & Variants"
                                >
                      ...
                      masterProductsList.map((prod) => (
                        <tr key={prod.id} className="hover:bg-slate-800/20 transition-colors">
                          <td className="py-4 pr-3">
                            <div className="flex items-center gap-3">
                              {prod.image_url && (
                                <img src={prod.image_url} alt={prod.name} className="w-10 h-10 object-contain rounded-lg bg-white/5 border border-slate-800 shrink-0" />
                              )}
                              <button
                                type="button"
            ...
            <div className="bg-[#0f172a] border border-slate-800 rounded-3xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
              ...
              <form id="editMasterProdForm" onSubmit={handleSaveMasterProduct} className="p-6 overflow-y-auto space-y-4 flex-1">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Master MRP (₹)</label>
                    <input
                      type="number"
                      step="any"
                      min="0"
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">SKU / Barcode</label>
                    <input
                      type="text"
                      value={editProdSku}
                      onChange={(e) => setEditProdSku(e.target.value)}
                      placeholder="e.g. AASH-ATT-5KG"
                      className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2 text-sm text-slate-100 focus:outline-none focus:border-emerald-500 font-mono"
                    />
                  </div>
                </div>

                {/* Pack Size Specification & Variant Management */}
                <div className="p-4 bg-slate-950/70 rounded-2xl border border-slate-800/80 space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-[10px] font-bold text-slate-300 uppercase tracking-wider">Pack Size Specification</div>
                      <p className="text-[10px] text-slate-500">Current product pack configuration</p>
                    </div>
                    <div className="text-xs font-bold text-emerald-400 bg-emerald-500/10 px-2.5 py-1 rounded-lg border border-emerald-500/20">
                      Display: {editProdPackQty && editProdPackUnit ? packUnitPayloadFromInput(editProdPackQty, editProdPackUnit).unit : '—'}
                    </div>
                  </div>

                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-[10px] text-slate-400 uppercase font-semibold">Quantity Value</label>
                      <input
                        type="number"
                        step="any"
                        min="0"
                        ...
                        placeholder="e.g. 5"
                        className="w-full mt-1 bg-slate-900 border border-slate-800 rounded-xl px-3 py-2 text-sm text-slate-100 focus:border-emerald-500 outline-none"
                      />
                    </div>
                    <div>
                      <label className="text-[10px] text-slate-400 uppercase font-semibold">Unit Type</label>
                      <select
                        value={editProdPackUnit}
                        onChange={(e) => setEditProdPackUnit(e.target.value)}
                        className="w-full mt-1 bg-slate-900 border border-slate-800 rounded-xl px-3 py-2 text-sm text-slate-200 focus:border-emerald-500 outline-none cursor-pointer"
                      >
                        {PACK_UNIT_OPTIONS.map((opt) => (
                          <option key={opt.code} value={opt.code}>{opt.label}</option>
                        ))}
                  ...
                  {/* Sibling Pack Variants in this Family */}
                  <div className="border-t border-slate-800/60 pt-3">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Product Family Pack Variants</span>
                      <button
                        type="button"
                      ...
                      >
                        <Plus className="w-3.5 h-3.5" /> {showAddVariantForm ? 'Cancel New Variant' : 'Add Another Pack Variant'}
                      </button>
                    </div>

                    {/* Existing Siblings */}
                    {(() => {
                      const baseFamily = String(editProdName || editMasterProduct.name || '')
                        .replace(/\s+\d+(?:\.\d+)?\s*(kg|g|ml|l|L|pcs|pack|dozen)\s*$/i, '')
                        .trim()
                        .toLowerCase();
                      const siblings = masterProductsList.filter(
                        (p) =>
                          (p.brand || '').toLowerCase() === (editMasterProduct.brand || '').toLowerCase() &&
                          String(p.name || '')
                            .replace(/\s+\d+(?:\.\d+)?\s*(kg|g|ml|l|L|pcs|pack|dozen)\s*$/i, '')
                            .trim()
                            .toLowerCase() === baseFamily
                      );

                      if (siblings.length <= 1 && !showAddVariantForm) {
                        return (
                          <p className="text-[11px] text-slate-500 italic">
                            Only 1 pack size currently exists. Click "+ Add Another Pack Variant" above to create another size (e.g. 10 kg, 1 kg) for this product family.
                          </p>
                        );
                      }
                    ...
                    {showAddVariantForm && (
                      <div className="mt-3 p-3 bg-slate-900/90 rounded-xl border border-emerald-500/30 space-y-3">
                        ...
                        <div className="grid grid-cols-4 gap-2">
                          <div>
                            <label className="text-[9px] text-slate-400 font-semibold uppercase">Quantity</label>
                            <input
                              type="number"
                              step="any"
                              min="0"
                              ...
                              onChange={(e) => {
                                ...
                                if (!newVariantSku && editMasterProduct) {
                                  const prefix = (editProdBrand.trim() || editMasterProduct.brand || editProdName).slice(0, 4).toUpperCase().replace(/[^A-Z0-9]/g, '');
                                  setNewVariantSku(`${prefix}-${e.target.value}${newVariantUnit}`.toUpperCase());
                                }
                              }}
                              className="w-full mt-1 bg-slate-950 border border-slate-800 rounded-lg px-2.5 py-1.5 text-xs text-slate-100 focus:border-emerald-500 outline-none"
                            />
                          </div>
                          <div>
                            <label className="text-[9px] text-slate-400 font-semibold uppercase">Unit</label>
                            <select
                              ...
                              onChange={(e) => {
                                ...
                                if (newVariantQty) {
                                  const prefix = (editProdBrand.trim() || editMasterProduct.brand || editProdName).slice(0, 4).toUpperCase().replace(/[^A-Z0-9]/g, '');
                                  setNewVariantSku(`${prefix}-${newVariantQty}${e.target.value}`.toUpperCase());
                                }
                              }}
                              className="w-full mt-1 bg-slate-950 border border-slate-800 rounded-lg px-2 py-1.5 text-xs text-slate-200 focus:border-emerald-500 outline-none cursor-pointer"
                            >
                              {PACK_UNIT_OPTIONS.map((opt) => (
                                <option key={opt.code} value={opt.code}>{opt.label}</option>
                              ))}
                          ...
                          <div>
                            <label className="text-[9px] text-slate-400 font-semibold uppercase">MRP (₹)</label>
                            <input
                              type="number"
                              step="any"
                              min="0"
                          ...
                          <div>
                            <label className="text-[9px] text-slate-400 font-semibold uppercase">Selling Price (₹)</label>
                            <input
                              type="number"
                              step="any"
                              min="0"
                        ...
                        <div className="flex items-center justify-between pt-1">
                          <div className="text-[11px] text-slate-400">
                            Will create SKU: <span className="font-mono text-emerald-300">{newVariantSku || 'Auto-generated'}</span>
                          </div>
                          <button
                            type="button"
                            disabled={newVariantSaving || !newVariantQty || !newVariantMrp || !newVariantPrice}
                            onClick={handleCreateSiblingVariant}
                            className="px-4 py-1.5 bg-emerald-500 hover:bg-emerald-400 disabled:opacity-50 text-slate-950 rounded-lg text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5"
                          >
                            {newVariantSaving ? 'Creating...' : '+ Create Variant SKU'}
                          </button>
                        </div>
                      </div>
                    )}
